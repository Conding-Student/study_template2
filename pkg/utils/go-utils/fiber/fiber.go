...
func GetJWTClaim(key string) map[string]any {
	return GetJWTClaims()[key].(map[string]any)
}
...
// GenerateJWTSignedString generates a JWT token with given claims and signs it with the secret key
func GenerateJWTSignedString(claimMaps fiber.Map) (string, error) {
	...
	// Add claims from the input map
	for key, value := range claimMaps {
		claims[key] = value
	}

	// Sign the token with the secret key
	signedToken, err := token.SignedString(jwtConfig.SecretKey)
	if err != nil {
		return "", err
	}

	// Optionally set a cookie with the token if configured
	...
	encryptedToken, err := encryptDecrypt.Encrypt(signedToken, utils.GetEnv("SECRET_KEY"))
	if err != nil {
		return "", err
	}
