...
func GetCancelledLoanSummary(c *fiber.Ctx) error {
	...
	if err != nil {
		logs.ErrorLogs(staffID, module, "Failed to generate Summary of Loan Cancelled. "+dateRange+"\nError Details: "+err.Error())
		return c.Status(401).JSON(response.ResponseModel{
			RetCode: "401",
			Message: status.RetCode401,
			Data: errors.ErrorModel{
	...
	summaryOfLoans, err := GetCancelledSummary(staffID, desigInt, summaryRequest.StartDate, summaryRequest.EndDate)
	if err != nil {
		logs.ErrorLogs(staffID, module, "Failed to generate Summary of Loan Cancelled. "+dateRange+"\nError Details: "+err.Error())
		return c.Status(401).JSON(response.ResponseModel{
			RetCode: "401",
	...
	branchName, err := GetBranchName(staffID, desigInt)
	if err != nil {
		logs.ErrorLogs(staffID, module, "Failed to generate Summary of Loan Cancelled. "+dateRange+"\nError Details: "+err.Error())
		return c.Status(401).JSON(response.ResponseModel{
			RetCode: "401",
	...
	fullname, err := GetGeneratedBy(staffID)
	if err != nil {
		logs.ErrorLogs(staffID, module, "Failed to identify staff generating Summary of Loan Cancelled. "+dateRange+"\nError Details: "+err.Error())
		return c.Status(401).JSON(response.ResponseModel{
			RetCode: "401",
			Message: status.RetCode401,
			Data: errors.ErrorModel{
	...
	logs.CardIncAuditTrail(staffID, module, "The user successfully fetched Summary of Loan Cancelled "+dateRange)
	return c.Status(200).JSON(response.ResponseModel{
		RetCode: "200",
		Message: "Successfully fetch summary of reports",
		Data: fiber.Map{
			"reportstitle":  "Summary of Loan Cancelled",
